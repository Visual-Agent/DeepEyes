...
openai_api_base = "http://10.39.10.76:8000/v1"
        ...
        try:
            while '</answer>' not in response_message:
                ...
                if start_token in response_message:
                    ...
                    rleft, rtop, rright, rbottom = map_box(left, top, right, bottom, width_scale, height_scale, ori_width, ori_height)
                    cropped_image = pil_img.crop((rleft, rtop, rright, rbottom))
                    new_w, new_h = smart_resize((rright - rleft), (rbottom - rtop), factor=IMAGE_FACTOR)
                    cropped_image = cropped_image.resize((new_w, new_h), resample=Image.BICUBIC)
                    cropped_pil_image = encode_pil_image_to_base64(cropped_image)
