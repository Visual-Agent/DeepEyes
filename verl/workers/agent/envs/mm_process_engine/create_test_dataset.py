...
if __name__ == '__main__':
    ...
    jsonl_path = "/fs-computility/mabasic/yangminghao/data/vstar-bench/test_questions.jsonl"
    jsonl_data = []
    ...
    def make_map_fn(split):
        ...
        def process_fn(example, idx):
            ...
            problem = example.pop('text')
            prompt = '<image>\n' + '\nQuestion:\n' + problem + instruction_prompt
            answer = example.pop('label')
            image_url = example.pop('image')
            images = []

            image_path = os.path.join("/fs-computility/mabasic/yangminghao/data/vstar-bench", image_url)
    ...
    dataset = raw_dataset.map(function=make_map_fn('test'), with_indices=True, num_proc=8)

    local_dir = args.local_dir
    hdfs_dir = args.hdfs_dir

    dataset.to_parquet(os.path.join(local_dir, 'vl_agent_V3_test_box_new.parquet'))
